(() => {
  ...
  async function openFile(file) {
    ...
    try {
      const resp = await fetch("/api/edit/session", { method: "POST", body: form });
  ...
  async function loadPage() {
    ...
    pageImage.src = `/api/edit/session/${sessionId}/page/${currentPage}?t=${Date.now()}`;
    clearSelection();
  }
  ...
  $("applyBtn").addEventListener("click", async () => {
    ...
    try {
      const resp = await fetch(`/api/edit/session/${sessionId}/apply`, {
        method: "POST",
  ...
  $("downloadBtn").addEventListener("click", () => {
    if (!sessionId) return;
    window.location.href = `/api/edit/session/${sessionId}/download`;
  });
  ...
  async function inspectAndSnap(pdfRect) {
    try {
      const resp = await fetch(`/api/edit/session/${sessionId}/inspect`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
